...
`tpt-dev-dx` is a Cargo workspace of independent, production-ready Rust crates focused on eliminating boilerplate in Rust backend development and testing. Each crate is published separately to crates.io. See `spec.txt` for the original design rationale behind each crate (problem solved, design details) — read it before making architectural changes to a crate.
...
# Test a single crate
cargo test -p tpt-faker-rs

# Test a single test by name
cargo test -p tpt-faker-rs some_test_name
...
- **`tpt-faker-rs`** + **`tpt-faker-rs-derive`** — strongly-typed, realistic mock data generation (e.g. Luhn-valid card numbers, locale-aware names, valid ISO dates) via a derivable `Fake` trait. `tpt-faker-rs-derive` is the internal proc-macro crate implementing the derive; not meant to be depended on directly. Optional `serde` feature.
